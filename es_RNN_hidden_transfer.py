...
def BCE_loss(y,p):
    return np.mean(-y*np.log(p + 0.0000000001) - (1 - y)*np.log(1 - p + 0.0000000001))
...
def evluate_func_test(data):
    model, target_model, params =  data
    model.set_model_params(params)
    model.reset()
    target_model.reset()
    loss_cum = 0
    Xs, labels  = recurrency_label(50)
...
def evluate_func2(data):
    model, target_model, params, n_iter =  data
    model.set_model_params(params)
    model.reset()
    target_model.reset()
    loss_cum = 0
    Xs, labels  = recurrency_label(n_iter)
