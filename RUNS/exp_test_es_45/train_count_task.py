...
def count_task(seq_len, train_x):
...
def evluate_func(data):
    model, params, train_X, n_batches =  data
    ...
    for batch in range(n_batches):
        ...
        Xs, labels  = count_task(task_len, train_X[batch])
...
train_X = [np.random.randint(2, size=[1,10]) for batch_ in range(100)]
...
#             fit_func((model,solutions[i]))
        seed = 1
        fitness_list = pool.map(fit_func, [(model,solutions[i], train_X, 100) for i in range(solver.popsize)])
        ...
        if args.algo == 'ga':
            ...
            fitness_list_2 = pool.map(fit_func, [(model,solutions[elite_idx], train_X, 10) for elite_idx in elite_idxs])
...
#             #print('----------------------RESET ELITES')
        ...
        if -result[1] <= 0.001:
            ...
            with open(filename, 'wt') as out:
                ...
                if args.algo == 'ga':
                    new_elite_params = solver.elite_params
                new_elite_rewards = pool.map(fit_func, [(model,solver.elite_params[kk], train_X, 5) for kk in range(len(solver.elite_rewards))])
