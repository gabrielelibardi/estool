...
#             fit_func((model,solutions[i]))
        seed = 1
        fitness_list = pool.map(fit_func, [(model,solutions[i], seed, 10) for i in range(solver.popsize)])
        

        if args.algo == 'ga___ll':
            ...
            fitness_list_2 = pool.map(fit_func, [(model,solutions[elite_idx], seed, 100) for elite_idx in elite_idxs])
